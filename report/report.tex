...
Table \ref{table2} summarizes the missclassification error and 95\% confidence intervals for the testing, validation, and training data sets. Although the training error confidence interval is based on more samples, and thus tighter, the testing error confidence interval provides a more realistic estimate of the true error of the two hidden node neural network classifier as applied to unseen data. This is because the network may be overfit to the particular characteristics of the training data samples which might not all be representative of the overall class of all possible input data. Therefore, the error we measure using the the testing data set (which was not used to train the algorithm) should give us a more realistic sense regarding how the classifier will perform on new data. 
